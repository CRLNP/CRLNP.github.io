...
- Très efficace pour la création de présentations et de rapports (`Rmarkdown`, Présentation à venir!)
...
- Les packages doivent être **téléchargés**(`install.packages()`) une seule fois, mais **chargés** (`library()`) à chaque session
...
- Utilisation implicite de la fonction `print(objet)`
```{r, echo = TRUE}
print(aa)  
```
...
### 3 opérateurs d'assignation (<em>assignment</em>) - Opérateurs qui permettent de créer un objet: `<-`, `->`, `=` (veut dire *gets*)
...
# Argument 2: lui dit d’ignorer les valeurs manquantes dans le calcul - la valeur par défaut est `FALSE`
...
En plus du **mode** définissant le format de leurs éléments/composantes, les objets sont aussi caractérisés par différentes **structures**: 1. les vecteurs, 2. les listes, 3. les matrices - à deux dimensions ou plus, 4. ie array - et 5. les tableaux (dataframe). Ce sont donc 5 différents types de contenants ayant chacun leurs propriétés.  
<center>
![https://devopedia.org/r-data-structures](C:/Users/p0373489/Desktop/AtelierR/markdown/types.jpg)
...
![https://mgimond.github.io/ES218/Week02a.html](C:/Users/p0373489/Desktop/AtelierR/markdown/tobjet2.jpg)
...
- Dans le fond, tout est composé de vecteurs – par exemple les tableaux sont composés d'e'un regroupement de vecteurs (variables).
...
- Un vecteur de plusieurs éléments numériques et une valeur manquante
```{r, echo = TRUE}
bb <- c(5, 23, 89, NA)
bb
```
...
- À la création d’un facteur, les valeurs sont regroupées en modalités (niveaux) fixes, il ne peut y en avoir d’autres.
- On peut toutefois modifier les niveaux permis et leur ordre avec la fonction ou l’argument `levels()`.
- Les niveaux sont ordonnés alphabétiquement par défaut mais les valeurs individuelles sont stoquées en format integer, débutant par 1 (ex: "femme"= 1, "homme"= 2).
- Les facteurs ont un attribut levels qui les distinguent des vecteurs réguliers:
  ...
  + L’argument `Labels` permet de modifier les libellés de niveaux (optionnel).
...
- **À noter**:
  + Parce que les valeurs sont stoquées sous forme numérique et que les niveaux ne sont qu’un attribut textuel, les facteurs sont considérés comme étant plus économiques en terme d’espace
  + Ils peuvent être plus contraignants à manipuler, mais très pratiques pour plusieurs fonctions.
...
- Souvent utilisées comme argument à une fonction.
...
```{r}
cc <- matrix(c("a", "b", "c", "d"), nrow=2, ncol=2)
...
- C’est donc un vecteur à plus de deux dimensions avec une classe implicite *array*.
...
- C’est donc une **liste bidimentionnelle** avec des colonnes - `ncol()` et des lignes - `nrow()`,  donc 2 dimensions - `dim()`.
...
```
  ...
  + Plusieurs fonctions comme `merge()`, `cbind()` et `rbind()` permettent de fusionner des colonnes, des lignes et des tables de données.
...
- Les objets ont aussi une `classe()` : c’est une propriété un peu plus complexe (issue du langage S) assignée à un objet pour définir l’information contenue ET déterminer la façon dont l’objet sera traité par certaines fonctions dites génériques (comme `print()`, `summary()`, `plot()`). Pour les vecteurs simples, la classe est implicite et correspond au mode. 
...
- Fonctionnalité permettant d'utiliser des **opérateurs** afin de sélectionner des valeurs en fonction de différents critères.
...
- À noter:
    - Les doubles crochets sont surtout utilisés pour extraire le contenu de listes: on utilise`[]` pour sélectionner un ou plusieurs éléments d'une liste (toujours sous forme de liste) et `[[]]` pour accéder au contenu d'un seul élément d'une liste. 
...
- On reprend d'abord les vecteurs d'étudiants
```{r, echo = TRUE}
etudiants<-c("etudiant1", "etudiant2", "etudiant3", "etudiant4", "etudiant5") 
...
```{r, echo = TRUE}
francais[2] <- 78 # remplace la deuxième valeur du vecteur par 88
...
## 20. OUF! Trop compliqué le code!?

### Pas de panique, `R Commander` à la rescousse!
![Package Rcmdr](C:/Users/p0373489/Desktop/AtelierR/markdown/rcommander.jpg)

#### Simplement charger library("Rcmdr") dans **R** de base
...
**Le créateur lui-même en décourage l'utilisation, mais peut tout de même être utile au départ**
