...
## 3. **RStudio** (et autres IDE)  {.smaller}
...
- NB. Pas de mises à jour automatiques, il faut installer une nouvelle version et retélécharger les *packages* (ou déplacer le répertoire de l’ancienne version et faire `update.packages`). **RStudio** utilisera par défaut la version la plus récente sur le poste (peut se faire manuellement).
...
*Voir [Guide de bonnes pratiques](http://adv-r.had.co.nz/Style.html) de mise en forme de script (Advanced R by Hadley Wickham)*
...
# on crée une matrice avec la fonction matrix(). 
# On lui passe notre vecteur et spécifie en argument avec le nombre de lignes et de colonnes
...
- On les crée avec la fonction `data.frame()` - les variables textuelles seront automatiquement converties en **facteur** (sinon il faut modifier l'argument - `stringsAsFactors`)
...
```{r, results='asis'}
# Utilise un opérateur pour définir une condition de sélection qui retourne une liste de valeurs logiques
francais>70 


```
```{r}
# insère cette condition entre crochets pour sélectionner les valeurs qui remplissent la condition
